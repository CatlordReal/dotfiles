...
- call Codex through existing ChatGPT subscription by default
- isolate subprocess from global Codex/Caveman config and split large Markdown at safe boundaries
...
Provider controls: `CAVEMAN_COMPRESS_PROVIDER=codex|claude` (default `codex`), `CAVEMAN_COMPRESS_MODEL`, `CAVEMAN_COMPRESS_CHUNK_CHARS`, `CAVEMAN_COMPRESS_TIMEOUT_SECONDS`, and optional `CAVEMAN_CODEX_BIN`. Codex mode uses ChatGPT login, never requires API-key billing, and does not route through or modify Caveman 2 native-app proxy configuration.
