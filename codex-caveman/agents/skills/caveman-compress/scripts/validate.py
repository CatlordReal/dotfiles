...
NUMBER_REGEX = re.compile(
    r"(?<![\w])(?:\d{4}-\d{2}-\d{2}|v?\d+(?:[.,:]\d+)*(?:%|[A-Za-z]+)?)(?![\w])"
)
...
def extract_urls(text):
    return {url.rstrip(URL_TRAILING_PUNCTUATION) for url in URL_REGEX.findall(text)}
...
def extract_numbers(text):
    return Counter(NUMBER_REGEX.findall(text))
...
def validate_numbers(orig, comp, result):
    n1 = extract_numbers(orig)
    n2 = extract_numbers(comp)
    if n1 != n2:
        result.add_error(f"Numeric/date/version mismatch: lost={n1 - n2}, added={n2 - n1}")
...
def validate(original_path: Path, compressed_path: Path) -> ValidationResult:
    ...
    validate_numbers(orig, comp, result)
