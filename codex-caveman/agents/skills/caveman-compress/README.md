...
A memory-file compressor that supports Codex through an existing ChatGPT subscription and the upstream Claude path.
...
**Requires:** Python 3.10+ and Codex authenticated through ChatGPT (default), or Claude when explicitly selected.

Codex mode is persistent in this installation and processes large Markdown in safe chunks. It starts isolated ephemeral `codex exec` jobs, suppresses subprocess event streams, and reads only the final output file. This avoids recursive Caveman context and pipe/output stalls.

```bash
# Defaults shown
export CAVEMAN_COMPRESS_PROVIDER=codex
export CAVEMAN_COMPRESS_MODEL=gpt-5.6-sol
export CAVEMAN_COMPRESS_CHUNK_CHARS=12000
export CAVEMAN_COMPRESS_TIMEOUT_SECONDS=300
```
