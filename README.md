...
The installer detects apt, dnf, yum, pacman, and zypper; installs Neovim 0.11+, Node.js 18+, Python, Lua, .NET 10, compiler/parser tools, language servers, formatters, debuggers, and CLI tools (`bat`, `eza`, `fd`, `fzf`, `jq`, `ripgrep`, `starship`, `tmux`, `yazi`, `zoxide`, and `git-delta`); then restores plugins, Treesitter parsers, Mason packages, and writes a health log.

Existing `~/.config/nvim` is never overwritten by default. Use `--backup-existing-config` to move it to `$XDG_STATE_HOME/nvim-setup/backups/`, or `--no-config-install` to test the repository config in place.

Linux Neovim keeps Catppuccin Mocha local to Neovim. It has no Kitty, desktop appearance, SketchyBar, JankyBorders, SketchyVim, Finder, or Xcode integration.
