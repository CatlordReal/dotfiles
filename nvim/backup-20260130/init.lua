...
-- TODO: Use Noice as notification display and snacks notifier for history
...
require("lazy").setup({ -- kitty images
    {
        "3rd/image.nvim",
        opts = {
            backend = "kitty"
    ...
    }, -- mini files
    {
        ...
        config = function()
            require("mini.files").setup({
                ...
                content = {
                    -- Reserve icon space; SVGs rendered via image.nvim
                    prefix = function()
                        return "  "
                    end
                },
    ...
    }, -- File explorer
    {
        ...
        config = function()
            local ok_patch, patch = pcall(require, "neotree_catppuccin")
            if ok_patch and type(patch) == "table" and patch.setup then
                patch.setup()
            end
            --
            require("neo-tree").setup({
                ...
                -- let Oil own directory buffers; keep neo-tree manual
    ...
    build = ":TSUpdate",
    opts = {
        ensure_installed = { "lua", "c", "cpp", "c_sharp", "json", "markdown", "bash", "python", "html", "javascript",
            "typescript", "css" },
        highlight = {
            enable = true
        }
    ...
    },
    {
        ...
        config = function()
            require("mini.icons").setup({
                -- This makes anything using `nvim-web-devicons` (Neo-tree, etc.) pull icons from mini.icons
                mock_nvim_web_devicons = true,
            })

            -- Load your VS Code theme.json into mini.icons (must exist at this path)
            local ok, catppuccin_icons = pcall(require, "catppuccin_full_icon_logic")
            if ok then
                catppuccin_icons.load_theme(
                    vim.fn.stdpath("config")
                    .. "/icons/catppuccin/vscode-icons/dist/mocha/theme.json"
                )
                catppuccin_icons.enable_image_icons({
                    width = 2,
                    height = 1,
                    col_offset = 3,
                })
    ...
    cmd = { "OverseerOpen", "OverseerRun", "OverseerToggle", "OverseerBuild", "OverseerQuickAction", "OverseerClearCache" },
...
}, { "kkharji/sqlite.lua" }, -- DAP virtual text
    {
    ...
    }, -- neotest
    {
        ...
        dependencies = { "nvim-lua/plenary.nvim", "nvim-treesitter/nvim-treesitter", "nvim-neotest/nvim-nio",
            "nvim-neotest/neotest-python", "Issafalcon/neotest-dotnet" },
        config = function()
            require("neotest").setup({
                adapters = { require("neotest-python")({}), require("neotest-dotnet")({}) }
            })
        end
...
-- File explorers
-- vim.keymap.set("n", "<leader>e", "<cmd>Neotree toggle filesystem left<CR>", { silent = true, desc = "File Explorer" })
-- nvim-tree toggle keymap is defined within the plugin setup, so avoid duplicating it here
vim.keymap.set("n", "<leader>nt", "<cmd>Neotree toggle filesystem left reveal_force_cwd<CR>", { silent = true, desc = "Neo-tree" })
...
vim.keymap.set("n", "<leader>tv", "<cmd>ToggleTerm<cr>", {
    silent = true,
    desc = "Terminal"
})
...
-- Database keymaps
vim.keymap.set("n", "<leader>qo", "<cmd>DBUI<CR>", {
    desc = "Open DBUI"
})
vim.keymap.set("n", "<leader>qc", "<cmd>DBUIClose<CR>", {
    desc = "Close DBUI"
})
vim.keymap.set("n", "<leader>qr", "<cmd>DBUIRename<CR>", {
    desc = "Rename Connection"
})
vim.keymap.set("n", "<leader>qs", "<cmd>DBUISaveQuery<CR>", {
    desc = "Save Query"
})
...
wkr.add({                  -- File explorer
    ...
    "<leader>bv",
    desc = "Vertical split with other buffer"
}, {
    "<leader>bh",
    desc = "Horizontal split with other buffer"
}, -- Harpoon bookmarks
    {
        "<leader>h",
        group = "Harpoon"
...
-- Dotnet running
vim.keymap.set("n", "<leader>rd", "<cmd>Dotnet run<CR>", {
    desc = "Dotnet Run Project"
})
...
vim.keymap.set("n", "<leader>bV", function()
    ...
    desc = "Vertical split with chosen buffer"
})
...
vim.keymap.set("n", "<leader>bH", function()
    ...
    desc = "Horizontal split with chosen buffer"
})
...
local function _pick_other_listed_buffer(current, preferred)
    local function ok(buf)
        return buf and vim.api.nvim_buf_is_valid(buf) and vim.bo[buf].buflisted and buf ~= current
    end
...
vim.g.easy_dotnet_disable_workspace_edit = true

vim.keymap.set("n", "<leader>nt", "<cmd>Neotree toggle filesystem left<CR>", {
