...
function! db#adapter#sqlite#tables(url) abort
  return db#systemlist(db#adapter#sqlite#command(a:url) + [
        \ '-noheader',
        \ "SELECT name FROM sqlite_master WHERE type IN ('table', 'view') AND name NOT LIKE 'sqlite_%' ORDER BY CASE type WHEN 'table' THEN 0 ELSE 1 END, name"
        \ ])
endfunction
