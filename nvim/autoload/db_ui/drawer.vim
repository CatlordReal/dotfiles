...
function! s:drawer.populate_tables(db) abort
  ...
  if !has_key(a:db, 'views')
    let a:db.views = {'expanded': 0, 'items': {}, 'list': []}
  endif
  let a:db.views.list = []
  if empty(a:db.conn)
    return a:db
  ...
  if a:db.scheme =~? '^sqlite'
    call self.populate_sqlite_views(a:db)
  endif
...
function! s:drawer.populate_sqlite_views(db) abort
  let view_names = db#systemlist(db#adapter#sqlite#command(a:db.conn) + [
        \ '-noheader',
        \ "SELECT name FROM sqlite_master WHERE type = 'view' AND name NOT LIKE 'sqlite_%' ORDER BY name"
        \ ])

  let a:db.views.list = sort(map(copy(view_names), 'trim(v:val)'))
  call filter(a:db.views.list, 'v:val !=# ""')
  call filter(a:db.tables.list, 'index(a:db.views.list, v:val) < 0')
endfunction
...
function! s:drawer._render_schemas_section(db) abort
  if a:db.schema_support
    ...
    call self.add('Tables ('.len(a:db.tables.list).')', 'toggle', 'tables', self.get_toggle_icon('tables', a:db.tables), a:db.key_name, 1, { 'expanded': a:db.tables.expanded })
