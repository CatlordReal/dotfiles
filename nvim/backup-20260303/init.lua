...
local function apply_catppuccin_theme(flavour, opts)
    ...
    if type(_G.__refresh_catppuccin_borderflow) == "function" then
        local ok_borderflow, borderflow_err = pcall(_G.__refresh_catppuccin_borderflow)
        if not ok_borderflow then
            vim.notify("Borderflow refresh failed: " .. tostring(borderflow_err), vim.log.levels.WARN)
        end
    end
...
local ok_borderflow, borderflow = pcall(require, "catppuccin_borderflow")
if ok_borderflow then
    borderflow.setup({
        default_flavour = CATPPUCCIN_DEFAULT_THEME,
        flavour_getter = function()
            return normalize_catppuccin_flavour(vim.g.catppuccin_flavour or CATPPUCCIN_DEFAULT_THEME)
        end,
...
local function open_buffer_diagnostics_qf()
    ...
    if qf_open and qf.context and qf.context.kind == "buffer_diagnostics" then
        vim.cmd("cclose")
        return
    end
